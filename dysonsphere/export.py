...
def save(
    ...
    if saveMetadata:
        ...
        # embedReport: the human-readable report(s).  `report` is a *container* keyed by
        # section so future renderings hook in as siblings: `statistics` (only when there
        # are records) + `provenance` (always, so a stats-free chart still reads back a
        # sentence).  It's added to `_ds_block` *after* `_usermeta_json` is serialized, so it
        ...
        if embedReport and _provenance is not None:
            _report_sections = {}
            if _records:
                _report_sections["statistics"] = "\n\n".join(_render_report(r) for r in _records)
            _report_sections["provenance"] = _render_provenance(_provenance)
    ...
    try:
        ...
        for mode, suffix in [_background_map[b] for b in background]:
            ...
            if _report_sections is not None:
                # _inject_png_metadata prepends (each chunk lands right after IHDR), so inject
                # in reverse to leave the sections in section order — matching the JSON/SVG.
                for _section, _text in reversed(_report_sections.items()):
